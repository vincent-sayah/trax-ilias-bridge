...
function handleStatementsAggregate(array $config, string $method, string $client, ?string $storeFromUrl): void
{
    ...
    $headers = forwardHeaders(['authorization', 'accept', 'x-experience-api-version']);
    $headers[] = BYPASS_HEADER;
    $headers[] = 'X-Trax-Ilias-Bridge-Version: ' . BRIDGE_VERSION;
...
function aggregateForIlias(array $statements, array $pipeline): array
{
    ...
    foreach ($pipeline as $stage) {
        ...
        if (isset($stage['$sort']) && is_array($stage['$sort'])) {
            $sortStages[] = $stage['$sort'];
            continue;
        }

        if (isset($stage['$skip'])) {
            $skip = max(0, (int)$stage['$skip']);
            continue;
        }

        if (isset($stage['$limit'])) {
            $limit = max(0, (int)$stage['$limit']);
            continue;
        }

        if (isset($stage['$group']) && is_array($stage['$group'])) {
            $group = $stage['$group'];
            if (isset($group['_id']) && is_string($group['_id']) && str_starts_with($group['_id'], '$')) {
                $distinctGroupField = substr($group['_id'], 1);
            }
        }
    }

    foreach ($sortStages as $sort) {
    ...
    if ($distinctGroupField !== null) {
        ...
        $out = [];
        foreach ($docs as $doc) {
            foreach (pathValues($doc, $distinctGroupField) as $value) {
                if ($value === null || $value === '') {
                    continue;
                }
